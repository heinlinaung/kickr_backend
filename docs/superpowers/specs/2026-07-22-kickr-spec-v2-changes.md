...
`GroupMember`: `groupId`, `userId`, `role` (owner|admin|member), `status` (pending|approved), `joinedAt`. No separate Invitation collection — pending members ARE the invitations.
...
### 4.4 Event templates (§4.5 "schedule events / templates")
...
### 4.5 Public events / discovery (§4.6)
...
### 4.6 Shuffle integration (§4.5 preparation)
